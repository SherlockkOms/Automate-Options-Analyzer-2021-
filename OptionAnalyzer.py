...
for i in range(num_shares):
    ...
    try:
        ...
        b[i][5] = s["records"]["data"][arg]["CE"]["bidprice"]
    except:
        logf.write("\nNo CE for : {0}".format( str(b[i][4])))
        #print("no CE")
    try:
        b[i][6] = s["records"]["data"][arg]["PE"]["bidprice"]
    except:
        logf.write("\nNo PE for : {0}".format( str(b[i][4])))
        #print("no PE")
    ...
    b[i][16] = int(b[i][4] * (1 - int(how_far)/100))
    b[i][20] = int(b[i][4] * (1 + int(how_far)/100))
    print(b[i][16],b[i][20])
    ...
    #b[i][19] = round((float(b[i][17]) / float(b[i][16]) / int(margin) / int(days_left) * 365)*100, 2)
    ...
    print("#####")
    #b[i][20]=last_sp
    #b[i][21]=s["records"]["data"][l]["CE"]["bidprice"]
    ...
    if len(s) == 0:
        ...
        if b[i][16] == s["records"]["data"][0]["strikePrice"]:
            try:
                b[i][17] = s["records"]["data"][0]["PE"]["bidprice"]
            except:
                logf.write("\nNo pe found for : {0}".format( str(b[i][16])))
                #print("no pe found")
                b[i][17]= 0
                ...
                cols_color1[i] = turns
                try:
                    ratio2 = s["records"]["data"][0]["PE"]["askPrice"]- s["records"]["data"][0]["PE"]["bidprice"]
                except:
                    logf.write("\nNo PE for : {0}".format( str(s["records"]["data"][0]['strikePrice'])))
                    #print("No PE found")
                    ratio2 = 0 

                diff11[i][turns-1] = ratio2
                print(b[i][17],cols_color1[i],diff11[i][turns-i])
        else:
            count = 1
            for x in reversed(range(0, idx1+1)):
                if int(b[i][16]) == int(s["records"]["data"][x]["strikePrice"]) and(last_thursday.strftime("%d-%b-%Y") == s["records"]["data"][x]["expiryDate"]):
                    try:
                        ...
                        logf.write("\nNo PE for : {0}".format( str(s["records"]["data"][x]['strikePrice'])))
                        #print("NO PE DATA")
                        continue
                    ...
                    ratio2 = s["records"]["data"][x]["PE"]["askPrice"]- s["records"]["data"][x]["PE"]["bidprice"]
                    print(last_price,ratio1,ratio2)

                    if float(last_price)<=10 and (ratio1<=0.10 or ratio2 <= 0.5):
                        b[i][17]= last_price
                        cols_color1[i] = count
                    ...
                    elif float(last_price)>10 and float(last_price)<=20 and (ratio1<=0.10 or ratio2 <= 1.0):
                        b[i][17]= last_price
                        cols_color1[i] = count
                        diff11[i][count-1] = ratio2
                        bid1[i][count-1] = s["records"]["data"][x]["PE"]["bidprice"]
                    ...
                    elif float(last_price)>20 and float(last_price)<=50 and (ratio1<=0.10 or ratio2 <= 2.0):
                        b[i][17]= last_price
                        cols_color1[i] = count
                        diff11[i][count-1] = ratio2
                        bid1[i][count-1] = s["records"]["data"][x]["PE"]["bidprice"]
                    ...
                    elif float(last_price)>50 and float(last_price)<=100 and (ratio1<=0.10 or ratio2 <= 5.0):
                        b[i][17]= last_price
                        cols_color1[i] = count
                        diff11[i][count-1] = ratio2
                        bid1[i][count-1] = s["records"]["data"][x]["PE"]["bidprice"]
                    ...
                    elif float(last_price)>100 and (ratio1<=0.10 or ratio2 <= 10.0):
                        b[i][17]= last_price
                        cols_color1[i] = count
                        diff11[i][count-1] = ratio2
                        bid1[i][count-1] = s["records"]["data"][x]["PE"]["bidprice"]
                        ...
                        if count <=2:
                            ...
                            bid1[i][count-1] = s["records"]["data"][x]["PE"]["bidprice"]
                            ask1[i][count-1] = s["records"]["data"][x]["PE"]["askPrice"]
                            b[i][17] = (ask1[i][np.argmin(diff11[i])]+bid1[i][np.argmin(diff11[i])])/2
        ...
        if b[i][20] == s["records"]["data"][len(s["records"]["data"])-1]["strikePrice"]:
                b[i][21] = s["records"]["data"][len(s["records"]["data"])-1]["CE"]["bidprice"]
                turns=1
                cols_color2[i] = turns
        else:
            count = 1
            for x in range(idx2, len(s["records"]["data"])):
                if int(b[i][20]) == int(s["records"]["data"][x]["strikePrice"]) and(last_thursday.strftime("%d-%b-%Y") == s["records"]["data"][x]["expiryDate"]):
                    try:
                        ...
                        logf.write("\nNo CE for : {0}".format( str(s["records"]["data"][x]['strikePrice'])))
                        #print(" NO CE DATA for the value")
                        b[i][21] = 0
                        continue
                    ...
                    ratio2 = s["records"]["data"][x]["CE"]["askPrice"]- s["records"]["data"][x]["CE"]["bidprice"]
                    print(last_price,ratio1,ratio2)
                    ...
                    if float(last_price)<=10 and (ratio1<=0.10 or ratio2 <= 0.5):
                        ...
                        print("Cant be done in turn",count)
                        print("OLD BI20",b[i][20])

                        if count <=2:
                            diff22[i][count-1] = ratio2
                            bid2[i][count-1] = s["records"]["data"][x]["CE"]["bidprice"]
                            ask2[i][count-1] = s["records"]["data"][x]["CE"]["askPrice"]
                            if idx2+count <= len(s["records"]["data"])-1:
                                b[i][20] = s["records"]["data"][idx2+count]["strikePrice"]
                            else:
                                logf.write("CE index out of bounds for {0}".format(b[i][0]))

                            for z in range(1,4):
                                if idx2+z <= len(s["records"]["data"])-1:
                            ...
                            bid2[i][count-1] = s["records"]["data"][x]["CE"]["bidprice"]
                            ask2[i][count-1] = s["records"]["data"][x]["CE"]["askPrice"]
                            b[i][21] = (ask2[i][np.argmin(diff22[i])]+bid2[i][np.argmin(diff22[i])])/2


    b[i][18] = round((float(b[i][17]) / float (b[i][16]))*100,2)
    
    b[i][19] = round(((float(b[i][17]) / float(b[i][16]) / int(margin) / int(days_left) * 365)*100)*100, 2)
    b[i][22]=round((float(b[i][21]) / int(b[i][20]))*100, 2)
    b[i][23]=round((float(b[i][21]) / float((b[i][20]) / int(margin) / int(days_left) * 365)*100)*100, 2)
    ...
    if j=='S':
        for i in range(2,val):
            ...
            if cols_color1[i-2]==2:
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='FF6C52', end_color='FF6C52') #Lightest red
            elif cols_color1[i-2]==3:
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='FF3E1C', end_color='FF3E1C') #LR
            elif cols_color1[i-2]==4:
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='AD1E05', end_color='AD1E05') #DR
    ...
    if j=='U':
        for i in range(2,val):
            ...
            if sh1[var].value >= 48.0:
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='274E13', end_color='274E13')    # DARK GREEN
                #wb.save('Names.xlsx')
    ...
    if j=='B':
        for i in range(2,val):
            ...
            if perc >=1 and perc <2:
                ...
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='CC0000', end_color='CC0000')      # DARK RED
                #wb.save('Names.xlsx')
            else:
                continue 
    ...
    if j=='C':
        for i in range(2,val):
            ...
            if perc >=1 and perc <2:
                ...
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='274E13', end_color='274E13')
                #wb.save('Names.xlsx')
            elif perc >-2 and perc <=-1:
                fill_var=sh1[var]
                ...
                fill_var=sh1[var]
                fill_var.fill = PatternFill(fill_type='solid', start_color='CC0000', end_color='CC0000')
                #wb.save('Names.xlsx')
            else:
                continue  
...
for i in two_d:
    for j in range(2,val):
        try:
            ...
            sh1[i+str(j)] = round(sh1[i+str(j)].value,2)
        except:
            pass
